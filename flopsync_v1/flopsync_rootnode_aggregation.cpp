...
//	int timeant = jitterAbsorption+receiverTurnOn+w+packetTime;
	int timeant = w;
        rtc.setAbsoluteWakeup(frameStart+txtime-timeant);
	rtc.sleepAndWait();
        rtc.setAbsoluteWakeup(frameStart+txtime-timeant+jitterAbsorption);
	...
	unsigned int receivetime;
        ...
        for(;;)
        {
            ...
            if(timeout) break;    
            nrf.readPacket((void*)&p);
	    receivetime=rtc.getValue();
		if (p.check ==0xabc) break;
        }
        timer.stopTimeoutTimer();
        nrf.setMode(Nrf24l01::SLEEP);
	nrf.setPacketLength(4);

	if(timeout) puts("Timeout");
	else 
		{ 
		   printf("e=%d u=%d w=%d misspackets=%d unsynctime=%d sequence=%d rcvtime=%d \n",
		   p.e,p.u,p.w,p.misspackets, p.unsynctime,p.sequence, receivetime-frameStart-jitterAbsorption);
                }
    }
    return 0;
}
