...
const unsigned int minw=static_cast<int>(0.00006f*hz+0.5f);
    ...
    //Time for cc2520 to start its clock oscillator (200us)
    const unsigned int radioBoot=static_cast<int>(0.0002f*hz+0.5f);
    ...
    #ifndef USE_VHT
    //Additonal delay to absorb jitter (must be greater than pllBoot+radioBoot)
    const unsigned int jitterAbsorption=static_cast<int>(0.001f*hz+0.5f);
    #else //USE_VHT
    //Additonal delay to absorb jitter (must be greater than pllBoot+radioBoot)
    //Also needs to account for vht resynchronization time
