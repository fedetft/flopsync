...
while(<STDIN>)
{
	...
	if(/----/)
	{
		...
		my $miss=0;
		$miss=1 if($4);
		$miss=$t[$count] if($t[$count]>0);
		if($count==0) { $node1header="$1 $2 $3 $miss"; }
		elsif($count==1) { $node2header="$1 $2 $3 $miss"; }
		elsif($count==2) { $node3header="$1 $2 $3 $miss"; }
		$count++;
	} elsif(/node(\d).e=(-?\w+)( t=(\d+))?/) {
		my $value=$2;
		if($1==1) { push(@node1,$value); $t[0]=$4 if($4); }
		elsif($1==2) { push(@node2,$value); $t[1]=$4 if($4); }
		elsif($1==3) { push(@node3,$value); $t[2]=$4 if($4); }
